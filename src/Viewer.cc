...
void Viewer::Run()
{
    ...
    //创建一个名为“RO-MAP: Map Viewer”的窗口，窗口大小为1024x768
    pangolin::CreateWindowAndBind("RO-MAP: Map Viewer",1024,768);
    //初始化GLEW库，如果初始化失败，则抛出运行时错误。
    if(glewInit() != GLEW_OK)
        throw std::runtime_error("glewInit failed");

    //启用深度测试，这是3D鼠标处理程序所需的。
    ...
    //启用混合功能
    glEnable (GL_BLEND);
    //设置混合函数，以便在渲染透明对象时使用源颜色的alpha值和目标颜色的反alpha值进行混合
    glBlendFunc (GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

    //创建一个名为“menu”的控制面板，并设置其边界。
    pangolin::CreatePanel("menu").SetBounds(0.0,1.0,0.0,pangolin::Attach::Pix(175));
    //在面板中添加一个布尔变量“Follow Camera”，初始值为true，并且可以被用户修改。
    // menuFollowCamera是一个对象，用来检查当前的状态
    ...
    while(1)
    {
        //这行代码用于清除当前的颜色缓冲区和深度缓冲区
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

        mpMapDrawer->GetCurrentOpenGLCameraMatrix(Twc);
        //menuFollowCamera：用于控制相机是否跟随。
        //bFollow：当前是否位于跟随模式
        ...
        {
            //调用 s_cam.SetModelViewMatrix() 设置固定视角。
            //参数指定从点 (mViewpointX, mViewpointY, mViewpointZ) 看向原点 (0,0,0)，视角方向为 (0, -1, 0)。
        ...
        if(menuRotation)
        {
            if(initTtoc)
            {
                ...
                for(Object_Map* pObj : AllObjs)
                {
                    if(pObj->haveNeRF)
                    {
                        //mTow_NeRF.inverse()得到nerf坐标系到世界坐标系
