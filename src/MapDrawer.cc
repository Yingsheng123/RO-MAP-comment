...
/**
 * @brief 绘制当前相机的三维形状，通常用来在可视化环境中表示相机的位姿和方向。
*/
...
//std::transform 将一个存储 KeyFrame* 的容器 vpKFs 转换并插入到一个 std::map 容器 mappKFs 中，
//具体功能是以每个关键帧的时间戳 mTimeStamp 作为键，关键帧指针 pKF 作为值插入到 mappKFs 中。

//transform将一个范围 [vpKFs.begin(), vpKFs.end()) 的元素逐一应用某种转换逻辑，并将结果存入目标位置。
//指定插入位置为 mappKFs 容器的末尾。
    std::transform(
