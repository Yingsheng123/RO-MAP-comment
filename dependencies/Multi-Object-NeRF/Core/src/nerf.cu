    ...
    nerf::CPUMeshData& mesh = mCPUMeshData;
    {
        ...
        if (lock.owns_lock())
        {
            if(mesh.have_reslult)
            {
                ...
                //glVertexPointer（每个顶点包含3个坐标值，数据类型为 GL_FLOAT，数据间距（步长）为0（表示连续存储），数据指针（指向顶点数组 mesh.verts.data()）
                glVertexPointer(3,GL_FLOAT,	0,mesh.verts.data());	
                //每个颜色包含3个分量（RGB），数据类型为 GL_UNSIGNED_BYTE，数据间距为0（指向颜色数组 mesh.colors.data()）
                glColorPointer(3,GL_UNSIGNED_BYTE,0,mesh.colors.data());
                //数据类型为 GL_FLOAT，数据间距为0，数据指针指向法线数组 mesh.normals.data()
                glNormalPointer(GL_FLOAT, 0, mesh.normals.data());
                //图元类型，这里是三角形（GL_TRIANGLES)；索引数组的元素个数; 索引数据类型，这里是 GL_UNSIGNED_INT; 指向索引数组 mesh.indices.data()
                glDrawElements(GL_TRIANGLES,mesh.indices.size(),GL_UNSIGNED_INT,mesh.indices.data());
                //在绘制完成后，禁用之前启用的状态以避免影响后续的 OpenGL 渲染。
