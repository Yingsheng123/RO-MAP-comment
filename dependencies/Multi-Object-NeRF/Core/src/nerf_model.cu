...
/**
 * @brief
 * @param[in] raysInstance 射线和物体是否发生交集
 * @param[in] ObjInstanceId: 目标物体的实例 ID
*/
                            ...
                            float* fxfycxcy,int H,int W,uint8_t ObjInstanceId,bool useDepth)
{
    ...
    //如果实例 ID 不等于 0 且不等于目标物体的实例 ID，表示该射线不属于目标物体，跳过处理。
...
{
    //sample rays
    //生成均匀分布的随机数，填充 mBatchMemory.SampleXY 数组，表示当前批次射线的起始坐标，大小为mnRaysPerBatch * 2
    ...
    //异步初始化 InBboxRaysCounter 数组的值为 0
    //InBboxRaysCounter: 用于记录每条射线是否落在物体边界框内的计数器数组。
    //用来记录每条射线穿过物体的数量
    mBatchMemory.InBboxRaysCounter.memset_async(pStream,0);

    //Average distribution of sampling points
    //调用 GenerateRays 核函数生成射线的属性，例如射线的方向、颜色、目标深度等。
    //linear_kernel 是一个用于调用 CUDA 核函数的模板
    ...
    CUDA_CHECK_THROW(cudaStreamSynchronize(pStream));
    //根据每条射线生成多个采样点，并存储到 mBatchMemory.PointsInput 中
...
void NeRF_Model::GenerateMesh(cudaStream_t pStream, MeshData& mMeshData)
{
    ...
    Eigen::Vector3i res3i = GetMarchingCubesRes(mMesh.res, box);//mMesh.res为64分辨率
...
/***
 * @brief: 返回nerf中每一个点的密度
 * @param[in] res3i:三维分辨率，表示网格在 x、y、z 三个方向上的分辨率
 * @param[in] aabb:轴对齐包围盒，定义了计算网格的空间范围。
 * @param[in] pStream：CUDA 流，指定 CUDA 的计算流，用于异步操作。
*/
tcnn::GPUMemory<float> NeRF_Model::GetDensityOnGrid(Eigen::Vector3i res3i, const BoundingBox& aabb,cudaStream_t pStream)
{
    const uint32_t n_elements = (res3i.x()*res3i.y()*res3i.z());//计算点的总数
	tcnn::GPUMemory<float> density(n_elements,pStream);//分配内存大小

    //从网络模型中获取输出维度（通常包含 RGB 和密度值）。
    const uint32_t padded_output_width = mpNetwork->padded_output_width();
    ...
    //positions：用于存储网格点的坐标
    float* positions = std::get<0>(scratch);
    //mlp_out：用于存储网络模型的推理结果（RGB 和密度）。
    ...
    //调用 CUDA 核函数 generate_grid_samples_nerf_uniform，在给定分辨率范围内生成均匀分布的网格点坐标，并存储在 positions 中。
...
void NeRF_Model::TransCPUMesh(cudaStream_t pStream,CPUMeshData& cpudata)
{
    ...
    uint32_t n_verts = (uint32_t)mMesh.verts.size();
    verts.resize(n_verts * 3); //顶点
    vert_normals.resize(n_verts * 3);//法线
    vert_colors.resize(n_verts * 3);//颜色
